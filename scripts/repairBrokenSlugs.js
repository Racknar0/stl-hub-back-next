...
function detectBrokenSlug(slug) {
  ...
  return /[a-z]-[a-z]\b/.test(slug) && !/\d/.test(slug.split('-')[slug.split('-').length - 1]);
}
...
async function main() {
  ...
  for (const asset of assets) {
    ...
    if (expectedBase && asset.slug !== expectedBase) {
      ...
      if (slugWithoutSuffix !== expectedWithoutSuffix) {
        toFix.push({ id: asset.id, oldSlug: asset.slug, expectedSlug: expectedBase, title: asset.title });
      }
    }
  }
  ...
  for (const f of sample) {
    console.log(`  ID ${f.id}: "${f.oldSlug}" → esperado: "${f.expectedSlug}" (titulo: "${f.title?.slice(0,50)}")`);
  }
